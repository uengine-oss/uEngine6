...
<script>
    export default {
        ...
        mounted() {
            // this.monitor = true;
            console.log(this.instanceId, this.rootInstanceId)
        ...
        methods: {
            ...
            getInstance: function () {
                var me = this;
                var instance = {};
                this.$http.get(window.origin + "/instances/" + me.instanceId).then((result2)=>{
                    var defId = result2.data.defId.replace('.xml','.json')
                    // console.log(me.defName)
                  this.$http.get(window.origin + "/definition/raw/" + defId).then((result3)=>{
                  ...
                  this.$http.get(window.origin + "/instances/search/findFilterICanSee").then((result2)=>{
                    ...
                    for (var i in instances) {
                        if (instances[i] instanceof Object) {
                            ...
                            var selfLink = instances[i]._links.self.href
            ...
            loadStatus: function (definition) {
                ...
                this.$http.get(`${window.origin}/instance/${me.instanceId}/variables`)
                // me.$root.codi('instance{/id}/variables').get({id: me.instanceId})
                    .then(function (response) {
                        ...
                        me.definition = definition.data.definition;
                        console.log({"definitionFilledWithInstanceInfo": me.definition})
                    })
            },
            getActivity: function (tracingTag, definition) {
                var me = this;
                if (!definition.data.definition) definition.data.definition = me.definition;

                for (var key in definition.data.definition.childActivities[1]) {
                    if (definition.data.definition.childActivities[1][key]["tracingTag"] == tracingTag) {
                        return definition.data.definition.childActivities[1][key];
                    }
                }
            },
