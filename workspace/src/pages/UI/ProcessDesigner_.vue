  ...
  <div class="canvas-panel">
      <v-layout md-flex="20">
        <v-layout v-if="id && id.indexOf('@') > -1">
          <v-btn v-if="!isProduction" @click="markProduction" class="md-raised md-accent">Mark Production (rev
      ...
      <v-layout @contextmenu.native="openMenu" @mousedown.native="closeMenu">
        <div style="position: absolute; left:48%; top: 37%; z-index: 99999" v-if="loaded == false">
          <v-spinner :md-size="90" md-indeterminate style="z-index: 99999"></v-spinner>
        </div>
        ...
        <v-card v-if="!monitor" class="tools" style="top:100px;">
          <span class="bpmn-icon-hand-tool hands" _width="30" _height="30" v-bind:style="handsStyle"
            ...
            <v-tooltip md-direction="right">Drag On/Off</v-tooltip>
          </span>
        ...
        <v-card v-if="!monitor" class="tools" style="top:427px;">
          <span class="icons fa fa-undo" v-on:click="undo" style="margin-left:7px;">
            <v-tooltip md-direction="right">Undo</v-tooltip>
          </span>
          <span class="icons fa fa-repeat" v-on:click="redo" style="margin-left:7px;">
            <v-tooltip md-direction="right">Redo</v-tooltip>
          </span>
        </v-card>
        ...
        <v-layout>
          ...
          <v-layout v-if="!monitor">
            <v-input-container>
              ...
              <v-input v-model="definitionName" type="text"></v-input>
            </v-input-container>
          </v-layout>
          ...
          <v-layout v-if="versions">
            <v-input-container>
              ...
              <v-select v-model="selectedVersion" @change="changeVersion">
                <v-option v-for="version in versions.slice().reverse()" :value="version" :key="version">rev. {{version.ver}}
          ...
          <!--프로세스 정의-->
          <v-layout v-if="!monitor">
            <v-btn c lass="md-primary" id="processVariables" @click="openDefinitionSettings">
          ...
          <!--로케일-->
          <v-layout v-if="!monitor && definition" style="max-width: 200px;">
            <v-input-container>
              <label>Language</label>
              <v-select v-model="definition._selectedLocale" @change="changeLocale">
                <v-option value="ko">Korean</v-option>
          ...
          <!--프로세스 세이브-->
          <v-layout v-if="!monitor" style="margin-left: 30px;">
            <v-btn v-if="!monitor" class="md-fab md-primary md-mini" v-on:click="initiateProcess">
              <v-icon>play_arrow</v-icon>
            </v-btn>
            <v-btn v-if="!monitor" class="md-fab md-warn md-mini" @click="save">
          ...
          <v-layout v-if="monitor">
            <v-input-container>
              ...
              <v-input v-model="instanceName" type="text" readonly></v-input>
            </v-input-container>
          </v-layout>

          <v-layout v-if="monitor">
            <!--프로세스 변수-->
            <v-btn id="instanceVariables" @click="openInstanceVariables">Variables</v-btn>
            ...
            <!--담당자 변경-->
            <v-btn id="userPicker" @click="openUserPicker">Role Mappings</v-btn>
            <user-picker
              :id="instanceId"
              ref="userPicker"
...
//            }],
    ...
    methods: {
      ...
      deleteActivity: function () {
        ...
        for (var z1 = 0; z1 < me.definition.sequenceFlows.length; z1++) {
          if (me.definition.sequenceFlows[z1] != null) {
            if (me.definition.sequenceFlows[z1].selected == true) {
              me.definition.sequenceFlows[z1] = null;
            }
          }
        }

        for (var x1 = 0; x1 < me.definition.messageFlows.length; x1++) {
          if (me.definition.messageFlows[x1] != null) {
            if (me.definition.messageFlows[x1].selected == true) {
              me.definition.messageFlows[x1] = null;
            }
          }
        }
      ...
      loadStatus: function (definition) {
        ...
        axios.get('http://bpm.uengine.io:9090/instance/' + me.instanceId + '/variables')
