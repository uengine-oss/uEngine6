    ...
    >
      <template slot="properties-contents">
        ...
        <md-input-container>
          ...
          <md-checkbox
              v-if="activity.uriTemplate && activity.uriTemplate.indexOf('https://') == 0" v-model.number="activity.noValidationForSSL">Don't validate the certificate</md-checkbox>
        ...
        <md-subheader>Headers</md-subheader>

        <p v-for="(header, index) in activity.headers">
          <md-input-container>
            <label>Name</label>
            <md-input v-model="activity.headers[index].name"></md-input>
          </md-input-container>

          <md-input-container>
            <label>Value</label>
             <md-input v-model="activity.headers[index].value"></md-input>
          </md-input-container>

          <md-button @click="deleteHeader(index)">delete</md-button>

        </p>

        <md-button @click="addHeader">Add header</md-button>
...
<script>
  ...
  export default {
    ...
    methods: {
            ...
            addHeader: function(){
                if(!this.activity.headers)
                  this.activity.headers = [];

                this.activity.headers.push(
                  {name: '', value: '', _type: 'org.uengine.kernel.bpmn.HttpHeader'}
                );
            },

            deleteHeader: function(index){
              this.activity.headers.splice(index, 1);
            },
