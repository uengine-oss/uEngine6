...
public abstract class Transformer implements NeedArrangementToSerialize, Serializable {
	...
	public String getName() {
		if (name == null)
			return UEngineUtil.getClassNameOnly(getClass());
		return name;
	}
	...
	public Object letTransform(ProcessInstance instance, Map options) throws Exception {
		...
		/**
		 * variable 'resultForTransformers' stands for caching the result values from
		 * transformers within a transaction demarcation.
		 */
		...
		HashMap resultsForTransformers = (HashMap) instance.getProcessTransactionContext()
				.getSharedContext("resultForTransformers");

		/**
		 * call the prior transformers so that the transform action can be done
		 * recursively.
		 */
			...
			for (int i = 0; i < inputArguments.length; i++) {
				...
				for (int j = 0; j < argumentSources.size(); j++) {
					...
					if (argumentSource instanceof TransformerMapping) {
						...
						/**
						 * if there's cached result of this transformer, use it rather than transform
						 * again.
						 */
						/*
						 ...
						 * the transformer should implement how to resolve the output value through
						 * 'resolveOutputValue' method.
						 */
						parameters.put(inputArguments[i],
								/* resolveOutputValue(transformerArgument.getName(), */result/* ) */);
					} else if (argumentSource instanceof String) {// 프로세스 변수인 경우
		...
		/**
		 * if the in-parameter has value more than one, it should let transformers run
		 * with each values.
		 */
