...
public class ScopeActivity extends FlowActivity implements MessageListener {
	...
	public void addProcessVariable(ProcessVariable variable) {
		if (this.processVariableDescriptors == null) {
			this.processVariableDescriptors = new ProcessVariable[1];
			this.processVariableDescriptors[0] = variable;
		} else {
			ProcessVariable[] tempArray = new ProcessVariable[this.processVariableDescriptors.length + 1];
			System.arraycopy(this.processVariableDescriptors, 0, tempArray, 0, this.processVariableDescriptors.length);
			tempArray[this.processVariableDescriptors.length] = variable;
			this.processVariableDescriptors = tempArray;
		}
		alignProcessVariables();
	}
	...
	public boolean fireEventHandlers(ProcessInstance instance, int triggerType, EventMessagePayload emp)
			throws Exception {
	...
	public boolean onMessage(ProcessInstance instance, Object payload) throws Exception {
		if (payload instanceof EventMessagePayload) {
			...
			if (eh != null) {
				...
				ExecutionScopeContext esc = instance.issueNewExecutionScope(handlerActivity, triggerActivity,
						eh.getDisplayName().getText(GlobalContext.DEFAULT_LOCALE));
				instance.setExecutionScopeContext(esc);

				// if(instance.isRunning(eh.getHandlerActivity().getTracingTag())){
				// //new UEngineException("Event [" + eventName + "] is already running. Retry
				// when the event handling activity is finished.");
				// UEngineException eventIsAlreadyRunning = new UEngineException("Event [" +
				// eventName + "] is currently running. User may wait until the previous request
				// completes.");
	...
	public EventHandler getEventHandler(String eventName) {
		...
		if (eventHandlers != null)
			for (int i = 0; i < eventHandlers.length; i++) {
				if (eventHandlers[i] != null && eventHandlers[i].getName() != null
						&& eventHandlers[i].getName().equals(eventName))
	...
	public boolean registerToProcessDefinition(boolean autoTagging, boolean checkCollision) {
		...
		if (eventHandlers != null)
			for (int i = 0; i < eventHandlers.length; i++) {
				...
				// if(eventHandlingActivity.getProcessDefinition() == null ||
				// eventHandlingActivity.getParentActivity() == null){
	...
	public ValidationContext validate(Map options) {
		...
		// vc.add("[" + eventHandlingActivity.getName() + "] is not available as an
		// event handler.");
		// }
		//
		// if(eventNames.containsKey(eventHandler.getName())){
		// vc.add(getActivityLabel() + "The event handler name
		// ["+eventHandler.getName()+"] is duplicated.");
		// }
		//
		// eventNames.put(eventHandler.getName(),"");
		// }
		// }
		// if( this.getIncomingSequenceFlows() == null ||
		// (this.getIncomingSequenceFlows() != null &&
		// this.getIncomingSequenceFlows().size() < 1) ){
