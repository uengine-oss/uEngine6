...
public class GlobalContext {
	...
	// public final static String PROPERTY_AXIS_WEBAPP_DIR =
	// "uEngine.axis.directory";
	...
	// public final static String PROPERTY_PREVIEW_SERVER_HOST =
	// "preview.server.host";

	// public final static String PROPERTY_FASTCAT_SERVER_HOST =
	// "fastcat.server.host";
	// public final static String PROPERTY_FASTCAT_SERVER_PORT =
	// "fastcat.server.port";
	// public final static String PROPERTY_FASTCAT_CONTEXT_ROOT =
	// "fastcat.context.root";
	// public final static String PROPERTY_FASTCAT_SEARCH_CONTEXTNAME =
	// "fastcat.search.contextname";
	...
	// public static final String GLOBAL_PROCESS_PATH =
	// GlobalContext.getPropertyString("global.process.name", "_global.process");
	public static String WEB_CONTEXT_ROOT = GlobalContext.getPropertyString("web.context.root", "/uengine-web");

	// public final static boolean useEJB =
	// "true".equals(GlobalContext.getPropertyString("use.ejb", "false"));
	// public final static boolean useManagedTransaction =
	// "true".equals(GlobalContext.getPropertyString("use.managedtransaction",
	// "false"));
	// public final static boolean useAutoUserTransactionDemarcation =
	// "true".equals(GlobalContext.getPropertyString("use.auto.usertransaction.demarcation",
	// "false"));
	// public final static boolean captureSystemOut =
	// "true".equals(GlobalContext.getPropertyString("capture.system.out",
	// "false"));
	public final static boolean logLevelIsDebug = "debug".equals(GlobalContext.getPropertyString("log.level", "prod"));
	// public final static boolean CONNECTION_LEAKAGE_DETECT =
	// "true".equals(GlobalContext.getPropertyString("connection.leakage.detect",
	// "false"));
	// public final static String FILE_SYSTEM_PATH =
	// GlobalContext.getPropertyString("filesystem.path", "./");

	// public final static String USERTRANSACTION_JNDI_NAME =
	// GlobalContext.getPropertyString("usertransaction.jndiname",
	// "UserTransaction");
	public final static String ACTIVITY_DESCRIPTION_COMPONENT_OVERRIDER_PACKAGE = GlobalContext
			.getPropertyString("activitydescriptioncomponent.override.package");
	public final static String DEFAULT_LOCALE = GlobalContext.getPropertyString("default.locale",
			Locale.getDefault().getLanguage());

	// public final static String DATASOURCE_JNDI_NAME =
	// GlobalContext.getPropertyString("datasource.jndiname", null);
	// public final static String JDBC_DRIVERCLASSNAME =
	// GlobalContext.getPropertyString("jdbc.driverClassName", null);
	// public final static String JDBC_URL =
	// GlobalContext.getPropertyString("jdbc.url", null);
	// public final static String JDBC_USERNAME =
	// GlobalContext.getPropertyString("jdbc.username", null);
	// public final static String JDBC_PASSWORD =
	// GlobalContext.getPropertyString("jdbc.password", null);
	// public final static String JDBC_VALIDATION_QUERY =
	// GlobalContext.getPropertyString("jdbc.validationQuery", null);
	// public final static int JDBC_MAX_ACTIVE =
	// Integer.parseInt(GlobalContext.getPropertyString("jdbc.maxActive", "20"));
	// public final static int JDBC_MAX_IDLE =
	// Integer.parseInt(GlobalContext.getPropertyString("jdbc.maxIdle", "10"));
	// public final static long JDBC_MAX_WAIT =
	// Long.parseLong(GlobalContext.getPropertyString("jdbc.maxWait", "-1"));

	protected static IComponentFactory componentFactory = new DefaultComponentFactory();

	public static void setComponentFactory(IComponentFactory factory) {
	 ...
	 * try{
	 * Class SerCls = Thread.currentThread().getContextClassLoader().loadClass(
	 * "org.uengine.components.serializers."+ mechanism);
	 * 
	 * Serializer ser = (Serializer)SerCls.getConstructor(new
	...
	public static void setDefaultLocale(String locale) {
		...
		if (messages != null && !messages.getLocale().getLanguage().equals(locale)) {
			messages = PropertyResourceBundle.getBundle("org.uengine.resources.messages", new Locale(locale),
					GlobalContext.class.getClassLoader());
		}
	}
	...
	public static final String getLocalizedMessage(String key, String defaultValue) {
		try {
			...
			if (messages == null)
				messages = PropertyResourceBundle.getBundle("org.uengine.resources.messages", locale,
						GlobalContext.class.getClassLoader());
	...
	public static final String getLocalizedMessageForWeb(String key, String language, String defaultValue) {
		try {
			if (!webMessageBundles.containsKey(language)) {
				...
				PropertyResourceBundle propertyResourceBundle = (PropertyResourceBundle) PropertyResourceBundle
						.getBundle("org.uengine.messages", locale, GlobalContext.getClassLoader());

				if (!propertyResourceBundle.getLocale().getLanguage().equals(language))
					return defaultValue;
	...
	// if(sqls==null){
	// try {
	// String resourceURL = "org/uengine/resources/sqls/" +
	// DAOFactory.getInstance(null).getDBMSProductName().toLowerCase()
	// +".properties";
	// URL url = null;
	...
	// }else{
	// System.out.println("Please check whether resource '" + resourceURL +"'
	// exists. It is very important to start uEngine.");
	// }
	// }catch (Exception e) {
	// e.printStackTrace();
	// }
	...
	public static ClassLoader getComponentClassLoader() throws Exception {
		...
		/*
		 * File stubDir = new File("."+ System.getProperty("file.separator") +
		 * "servicestubs" + System.getProperty("file.separator"));
		 * File[] fs = (stubDir).listFiles();
		 * Vector urlVector = new Vector();
		 * 
		 * System.out.println("GlobalContext::getServiceProvider stub directory = "+
		 * stubDir);
		 * if(fs.length > 0)
	...
	public static Class getComponentClass(String compName) throws Exception {
		...
		// Class SPCls =
		// Thread.currentThread().getContextClassLoader().loadClass("org.uengine.components.serviceproviders."+
		// serviceKey);
	...
	public static Class getXMLBindingClass(QName qname) throws Exception {
		 ...
		 * String absolutePath = "xmlbindings"+
		 * System.getProperties().get("file.separator")+ clsPackageName;
		 * 
	...
	// static{
	// try {
	// xpdSerializerInstance = (Serializer)
	// Class.forName(getPropertyString("default.serializer.class",
	// "org.uengine.components.serializers.XPDSerializer")).newInstance();
	// } catch (Exception e) {
	// xpdSerializerInstance = new
	// org.uengine.components.serializers.XPDSerializer();
	// }
	// }
	...
	// static final Serializer axisBeanSerializerInstance = new
	// org.uengine.components.serializers.AxisBeanSerializer();
	public static Serializer getSerializer(Class cls) {
		...
		// if(axisBeanSerializerInstance.isSerializable(cls)) return
		// axisBeanSerializerInstance;
	...
	public static Serializer getSerializer(String encodingStyle) {
		...
		try {
			Serializer ser = (Serializer) Thread.currentThread().getContextClassLoader()
					.loadClass("org.uengine.components.serializers." + encodingStyle + "Serializer").newInstance();
	...
	public static void serialize(Object obj, OutputStream os, ProcessVariable pd) throws Exception {
		...
		/*
		 * if(pd.getXmlBindingClassName()!=null){ //if the target variable is a business
		 * document
		 */
		if (pd.getQName() != null) {
	...
	public static Object deserialize(InputStream is, ProcessVariable pd) throws Exception {
		 ...
		 * System.out.
		 * println("GlobalContext::deserialize: the case that source is a business document"
		 * );
	...
	// public static void updateLog(int countId, String logid, long time) throws
	// Exception {
	// IDAO logDAO = GenericDAO.createDAOImpl("java:/uEngineDS", "update
	// BPM_PERFORMANCE set "+logid+"=?"+logid+" where countid=?countid",
	// IDAO.class);
	...
	public static <T> T getComponent(Class<T> clazz, Object[] objects) {
		try {
			return (T) componentFactory.getComponent(clazz, objects);
		} catch (Exception e) {
