...
public class ProcessDefinition extends ScopeActivity implements Serializable {
	...
	public Activity getActivity(String tracingTag) {
		...
		if (!wholeChildActivities.containsKey(tracingTag))
			return null;// throw new RuntimeException( new UEngineException("No such child activity
						// where tracing tag='" + tracingTag+"' in the process definition '" + getName()
						// + "(" + getBelongingDefinitionId() + "@" + getId() + ")'"));
	...
	public void addRevisionInfo(RevisionInfo revisionInfo) {
		if (revisionInfoList == null)
			revisionInfoList = new ArrayList();
	...
	public ActivityFilter[] getActivityFilters() {
			...
			if (filters != null && filters.size() > 0) {
				...
				if (org.uengine.util.UEngineUtil.isNotEmpty(defaultActivityFilterClsNames)) {
					...
					for (int i = 0; i < filterClsNames.length; i++) {
						...
						try {
							defaultActivityFilters[j] = (ActivityFilter) GlobalContext.loadClass(filterClsName)
									.newInstance();
							j++;
						} catch (Exception e) {
		...
		if (defaultActivityFilters == null)
			defaultActivityFilters = new ActivityFilter[] {};
	 ...
	 * throw new
	 * RuntimeException("this process definition is not allowed to be initiated by the first workitem."
	 * );
	 * 
	 ...
	 * try{
	 * ProcessManager pm =
	 * ProcessManagerFactory.create(getProcessTransactionContext());
	 * String versionId = pm.getProductionVersionId(spAct.getDefinitionId());
	 * ProcessDefinition spDef =
	 * ProcessDefinitionFactory.getDefinitionWithVersionId(versionId,
	 * getProcessTransactionContext());
	 * 
	...
	public ActivityReference getInitiatorHumanActivityReference(final ProcessTransactionContext ptc) {
		...
		// this.initiatorHumanActivity = (activityReference==null ? null :
		// (HumanActivity)activityReference.getActivity());
	...
	public Calendar getDueDate(ProcessInstance instance) throws Exception {
		// Calendar dueDate = (Calendar)instance.get(getTracingTag(),
		// HumanActivity.PVKEY_DUEDATE);
	...
	public ProcessDefinition() {
		...
		setName(GlobalContext.getLocalizedMessage("activitytypes.org.uengine.kernel.processdefinition.untitled",
				"Noname"));
	...
	protected boolean registerActivity(Activity act, boolean autoTagging, boolean checkCollision) {
		...
		if (wholeChildActivities == null)
			wholeChildActivities = new Hashtable();
		...
		if (tracingTagOfAct == null) {
			if (autoTagging) {
				...
				throw new RuntimeException(new UEngineException(
						"This definition is corrupt. One of child activity's tracingtag is null."));
		}
		...
		// ((ComplexActivity)act).getChildActivities().elements();
	...
	static public ProcessDefinition getDefinition(String defVerId, ProcessTransactionContext ptc) throws Exception {
		 ...
		 * try{
		 * java.io.FileInputStream fi = new java.io.FileInputStream(processDefinition +
		 * ".Bean" );
		 ...
		 * System.out.
		 * println("found and deserialized. process definition will be used is from a file."
		 * );
		 * 
		 * System.out.println("length of childs : " + pd.getChildActivities().size());
		 * }catch(Exception
		 * e){System.out.println("Although tried to find process in files, failed");}
		 * }
		 * 
	...
	protected void addMessageListener(String message, ProcessInstance instance, String scope) throws Exception {
		 ...
		 * else
		 * removeMessageListener(message, instance, scope); //void duplicated
		 * subscription
		 * 
		 * System.out.println("ProcessDefinition::addMessageListener.message = " +
		 * message);
		 * 
	...
	protected void removeMessageListener(String message, ProcessInstance instance, String scope) throws Exception {
		 ...
		 * 
		 * if(subscriber.getInstanceId().equals(instance.getInstanceId()) &&
		 * contextScope.equals(scope)){
		 * removeTargets.add(context);
		 * System.out.println("found duplicated message subscription!: message=" +
		 * message +", instanceid=" + instance.getInstanceId() + ", scope=" + scope);
		 * }
		 * }
		 * 
	...
	// TODO: hotspot
	protected Object fireMessage(String message, final ProcessInstance instance, Object payload, boolean payloadIsXML)
			throws Exception {
		...
		if (subscribedScopes != null)
			for (Enumeration enumeration = subscribedScopes.elements(); enumeration.hasMoreElements();) {
				 ...
				 * ProcessVariableDescriptor pvd =
				 * getProcessVariableDescriptor(outPV.getName());
				 * QName qname = pvd.getQName();
				 * 
				 * Serializer ser = GlobalContext.getSerializer(qname);
				 * Object objPayload = ser.deserialize(new
				 * java.io.StringBufferInputStream((String)payload));
				 * payload = objPayload;
				 * }
				 * }
				 */
				...
				if (!(targetActivity instanceof MessageListener))
					continue;
				...
				// message.equals(activityAsMessageListener.getMessage())){
		 ...
		 */
		// if(!ack)
		// throw new UEngineException("There is no subscribed listener for the message",
	...
	protected void reportFaultToMainProcess(ProcessInstance instance, FaultContext e) throws Exception {
		// if this process is subprocess, notify that it is completed to the main
		// process.
		...
		if (returningProcess != null && !returningProcess.equals("null")) {
			...
			if (returningActivity == null)
				throw new UEngineException(
						"subprocess activity is not found. maybe event handling subprocess is set having to returned.");
	...
	public void afterDeserialization() {
		...
		// EventHandler[] eventHandlers = super.getEventHandlers(); //anyhow now there's
		// no process definition having its event handlers since the process designer
		// isn't allowed to add event handlers.
	...
	public void flowControl(String command, ProcessInstance instance, String tracingTag) throws Exception {
		...
		if (command.equals("resume")) {
			...
			if (status.equals(STATUS_COMPLETED) || status.equals(STATUS_SKIPPED))
				...
				System.out.println("compensateTo cancelled...");
		}
	}
	...
	public ValidationContext validate(Map options) {
		 ...
		 * if(initiatorHumanActivity==null)
		 * ctx.
		 * add("Although this definition is set to be initiated by the first workitem, there's no initiating human activity."
		 * );
		 * }
		 */
		...
		ActivityForLoop findingLoopForTracingTagCollision = new ActivityForLoop() {
			...
			public void logic(Activity activity) {
				if (tracingTagMap.containsKey(activity.getTracingTag())) {
					...
					ctx.add(activity.getActivityLabel() + " TracingTag collision with "
							+ originalTTUser.getActivityLabel()
							+ ". You should choose another tag number for this activity (Change the definition source directly code in the 'XPD' Tab).");
				} else
	...
	public static ProcessDefinition create() {
		try {
			...
			return (ProcessDefinition) Thread
					.currentThread().getContextClassLoader().loadClass(GlobalContext
							.getPropertyString("processdefinition.class", "org.uengine.kernel.ProcessDefinition"))
					.newInstance();
	...
	// public static String getDefinitionVersionId(ProcessInstance instance, String
	// definitionId, int versionSelectionOption, ProcessDefinition definition)
	// throws Exception{
	...
	//
	// return getDefinitionVersionId(instance, pm, definitionId,
	// versionSelectionOption, definition);
	// }
	//
	// public static String getDefinitionVersionId(ProcessManagerRemote pm, String
	// definitionId, int versionSelectionOption, ProcessDefinition definition)
	// throws Exception{
	// return getDefinitionVersionId(null, pm, definitionId, versionSelectionOption,
	// definition);
	// }

	// public static String getDefinitionVersionId(ProcessInstance instance,
	// ProcessManagerRemote pm, String definitionId, int versionSelectionOption,
	...
	// versionId = pm.getProductionVersionIdAtThatTime(definitionId,
	// instance.getRootProcessInstance().getProcessDefinition().getModifiedDate().getTime());
	// break;
	// case VERSIONSELECTOPTION_PROD_VER_AT_INITIATED_TIME:
	// versionId = pm.getProductionVersionIdAtThatTime(definitionId,
	// definition.getStartedTime(instance).getTime());
	// break;
	// case VERSIONSELECTOPTION_JUST_SELECTED:
	// if(versionId==null)
	// throw new UEngineException("Even though the sub process activity is set to
	// use the sub processes' definition version with the just selected one, the
	...
	protected void beforeExecute(ProcessInstance instance) throws Exception {
		...
		if (dueDate == null)
			setDueDate(instance, instance
					.calculateDueDate(GlobalContext.getNow(instance.getProcessTransactionContext()), getDuration()));
	...
	// private Activity findStartActivityWithEventSynchronization(Activity activity)
	// throws Exception {
	// try {
	// if (activity instanceof ReceiveActivity && activity.getEventSynchronization()
	// != null) {
	// return activity;
	// }
	...
	// Activity result =
	// findStartActivityWithEventSynchronization(sequenceFlow.getTargetActivity());
	// if (result != null) {
	// return result;
	// }
	// }
	...
	// return null;
	// } catch (Exception e){
	// throw new UEngineException("Error when to find StartActivityWith
	// EventSynchronization: " + activity.getName(), e);
	// }
