...
public class Evaluate extends Condition {
	...
	public boolean isMet(ProcessInstance instance, String scope) throws Exception {
		...
		if (UEngineUtil.isNotEmpty(type) && type.equals("loopCount")) {
			Activity act = null;
			try {
			...
			if (value.getClass() == String.class)
				compVal = Integer.parseInt((String) value);
			else if (value.getClass() == Integer.class)
				compVal = (Integer) value;
			if ("==".equals(condition))
			...
			if (key.startsWith("[instance]")) {
				...
				ProcessVariable processVariable = instance.getProcessDefinition().getProcessVariable(key);
				if (processVariable == null) {
					throw new Exception("Process variable '" + key + "' does not exist.");
				}

				returnVal = processVariable.get(instance, "");
			}
			...
			if (compareVal instanceof String && "date".equalsIgnoreCase(type)) {
				...
				if (((String) compareVal).startsWith("[instance]")) {
					...
					// ObjectInstance returnValObjInst = (ObjectInstance)
					// MetaworksRemoteService.getInstance().getMetaworksType(compareVal.getClass().getName()
					// ).metaworks2Type().createInstance();
					...
					// if( key.equalsIgnoreCase( compareVal.getClass().getSimpleName() + "." +
					// fd.getName() ) ){
			...
			// ObjectInstance returnValObjInst = (ObjectInstance)
			// MetaworksRemoteService.getInstance().getMetaworksType(returnVal.getClass().getName()
			// ).metaworks2Type().createInstance();
			...
			// for(int j=0; j<fields.length; j++){
			// FieldDescriptor fd = fields[j];
			// if( key.equalsIgnoreCase( returnVal.getClass().getSimpleName() + "." +
			// fd.getName() ) ){
			...
			// return keyVal.equals(compareVal);
			// }
			// if(!( keyVal instanceof Comparable) && (condition.startsWith(">") ||
			// condition.startsWith("<")))
			// throw new UEngineException("The value type ["+ returnVal.getClass() +"]
			// cannot be compared.");
			...
			// compareResult =
			// ((Integer)Integer.parseInt(String.valueOf(keyVal))).compareTo((Integer)compareVal);
			// } else if (compareVal instanceof Long) {
			// compareResult =
			// ((Long)Long.parseLong(String.valueOf(keyVal))).compareTo((Long)compareVal);
			// } else if (compareVal instanceof Float) {
			// compareResult =
			// ((Float)Float.parseFloat(String.valueOf(keyVal))).compareTo((Float)compareVal);
			// } else if (compareVal instanceof Double) {
			// compareResult =
			// ((Double)Double.parseDouble(String.valueOf(keyVal))).compareTo((Double)compareVal);
			// } else {
			...
			if (value instanceof ProcessVariable) {
				ProcessVariable variable = (ProcessVariable) value;
			...
			if (returnVal instanceof Comparable) {
				...
				try {
					 ...
					 */
					if (compareVal instanceof Integer) {
						compareResult = ((Integer) Integer.parseInt(String.valueOf(returnVal)))
								.compareTo((Integer) compareVal);
					} else if (compareVal instanceof Long) {
						compareResult = ((Long) Long.parseLong(String.valueOf(returnVal))).compareTo((Long) compareVal);
					} else if (compareVal instanceof Float) {
						compareResult = ((Float) Float.parseFloat(String.valueOf(returnVal)))
								.compareTo((Float) compareVal);
					} else if (compareVal instanceof Double) {
						compareResult = ((Double) Double.parseDouble(String.valueOf(returnVal)))
								.compareTo((Double) compareVal);
					} else {
	...
	public String toString() {
		...
		// String returnVal = "instance.get(\""+ expression + "\"). " +
		// this.convertJavaCondition(condition, val.toString());
		String returnVal = "instance.get(\"" + expression + "\")"
				+ this.convertJavaCondition(condition, value.toString());
	...
	public ValidationContext validate(Map options) {
		...
		// ProcessVariable pv =
		// activity.getProcessDefinition().getProcessVariable(evaluate.getKey());
		// if(pv==null)
		// validationContext.addWarning(activity.getActivityLabel()+" unrecognized
		// process variable '" + evaluate.key + "' used in a condition expression" );
		// else{
		// try{
		// Class valueType;
		//
		...
		// valueType =
		// activity.getProcessDefinition().getProcessVariable(((ProcessVariable)evaluate.getValue()).getName()).getType();
		// }else
		...
		// }catch (Exception e) {
		// validationContext.addWarning(activity.getActivityLabel()+" value '" +
		// evaluate.getValue() + "' is not comparable to process variable '" +
		// evaluate.key + "'" );
		// }
		// }else{
		// validationContext.addWarning(activity.getActivityLabel()+" value '" +
		...
		if (key == null || key.equals("")) {
			validationContext.getErrorMessage().add("프로세스 변수를 참조하는 조건이 존재하지 않습니다.");
		}
		if (value == null || value.equals("")) {
			validationContext.getErrorMessage().add("조건에 해당하는 값이 존재하지 않습니다.");
		}
		if (condition == null || condition.equals("")) {
			validationContext.getErrorMessage().add("조건이 존재하지 않습니다.");
		}
