...
public class SubProcess extends ScopeActivity {
        ...
        if (getForEachRole() != null) { // if there is configured forEachRole, initiate sub processes for each role
            ...
            do {
                ...
                if (unInitiatedRoleMappings.containsKey(endpoint)) {
                    ProcessInstance thePI = initiateSubProcess(instance, thisRM, thisValue,
    ...
    protected void executeActivity(ProcessInstance instance) throws Exception {
        ...
        if (getForEachVariable() != null) { // if there's no mapped for each role setting, Multiple Instances is
                                            // ignored.
    ...
    private void executeActivity_CaseInParallel(ProcessInstance instance) throws Exception {
        ...
        if (getForEachRole() != null) { // if there is configured forEachRole, initiate sub processes for each role
            ...
            do {
                ...
                ExecutionScopeContext esc = instance.issueNewExecutionScope(this, this,
                        thisRM != null ? thisRM.getResourceName() : "<No Name>");
                instance.setExecutionScopeContext(esc);

                ProcessInstance thePI = initiateSubProcess(instance, thisRM, thisValue, isConnectedMultipleSubProcesses,
                        roleMapping.getCursor());
            ...
            do {
                ...
                ProcessInstance thePI = initiateSubProcess(instance, null, theValue, isConnectedMultipleSubProcesses,
                        pvv.getCursor());
        ...
        for (int i = 0; i < execScopes.size(); i++) {
            ExecutionScopeContext execScope = execScopes.get(i);
    ...
    public boolean onSubProcessReturn(ProcessInstance instance, final String subProcessExecutionScope)
            throws Exception {
        ...
        Object boolObject = new InParentExecutionScope() {
            ...
            public Object logic(ProcessInstance instance) throws Exception {
                ...
                // TODO: following values are not meaningful anymore. due to executionscope
                // designate the sub processes for now.
                Map subProcesses = new Hashtable();
                subProcesses.put(instance.getInstanceId(), instance);

                ////// Since Introducing VariablePointer, apply variable changes to the main
                ////// process execution scope is not required anymore.
                // applyVariableBindings(instance, spIds, subProcesses,
                ////// optionsForVariableMapping);
                ...
                if (isRunAndForget()) {
                    ...
                    //
                    // if(isConnectedMultipleSubProcesses){
                    // connectedNextSubProcessActivity.onEvent(EVENT_ONE_OF_PREV_SP_COMPLETED,
                    // instance, new Integer(orderOfCurrentlyCompletedSubProcessInstance));
                    // }
    ...
    private Set getUncompletedSubProcessIds(ProcessInstance instance) throws Exception {
        ...
        Vector completedSPIds = getSubprocessIds(instance, SUBPROCESS_INST_ID_COMPLETED);
        // 30@0
        Vector execScopes = getSubprocessIds(instance, "ExecutionScope");
                ...
                if (variableBindings != null)
                    for (int i = 0; i < variableBindings.size(); i++) {
                        ...
                        try {
                            ...
                            /***
                             * since new version will use VariablePointer join mechanism is not required
                             * anymore.
                             ***/
                            ...
                            //
                            // /*in parent
                            // context*/instance.setExecutionScope(instance.getMainExecutionScope());
                            // {
                            ...
                            // }
                            //
                            // instance.add("", vb.getVariable().getName(), valueOfSP,
                            // whereStands);//process multiple pv
                            //
                            ...
                            UEngineException richException = new UEngineException(
                                    "Error when to set the value [" + vb.getVariable() + "] from returned subprocess",
                                    e);
    ...
    protected void afterComplete(ProcessInstance instance) throws Exception {
        ...
        if (getForEachVariable() != null) {
            ...
            if (!isRunAndForget()) {
                // applyVariableBindings(instance, spIds, subProcesses, options); // don't need
                // anymore by introducing the VariablePointer.
    ...
    protected void transferValues(ProcessInstance instance, ProcessInstance subProcessInstance,
        ...
        if (variableBindings != null)
            for (int i = 0; i < variableBindings.size(); i++) {
                ...
                // instance.addDebugInfo(" to sub process' variable: ",
                // pvpc.getArgument().getText());
    ...
    public Vector<String> getSubprocessIds(ProcessInstance instance, String space) throws Exception {
        // Activity_0gf1pdp:0:_status:prop
    ...
    public void fireComplete(ProcessInstance instance) throws Exception {
        ...
        if (completable
                && instance.getProcessTransactionContext().getSharedContext(SUB_PROCESS_IS_BEING_INSERTED) == null) {
            if ("loop".equals(getMultipleInstanceOption())) {
