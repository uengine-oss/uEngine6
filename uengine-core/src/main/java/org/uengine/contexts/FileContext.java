...
public class FileContext implements java.io.Serializable, /* CommandVariableValue, */ Cloneable {
	...
	// return dependencyFieldValue!=null &&
	// ((Boolean)dependencyFieldValue).booleanValue();
	// }
	...
	public File getFileForEngine() {
		...
		String dir = GlobalContext.getPropertyString("filesystem.path",
				org.uengine.kernel.ProcessDefinitionFactory.DEFINITION_ROOT);
	...
	public String getFullSavingFTPURL(ProcessInstance instance) throws Exception {
		return "ftp://" + getFtpUserId() + ":" + getFtpPassword() + "@" + getFtpAddress() + "/" + getFtpDirectory()
				+ "/" + getSavingFileName(instance);
	}

	private String getSavingFileName(ProcessInstance instance) throws Exception {
		return (instance != null
				? instance.getProcessDefinition().evaluateContent(instance, getFtpFileNamePattern()).toString()
				: getFtpFileNamePattern());
	}

	// public void beforeOpen(ProcessInstance instance, String variableKey) throws
	// Exception {
	...
	// if(instance!=null){
	// theVariable =
	// instance.getProcessDefinition().getProcessVariable(variableKey);
	// defaultValue = (FileContext)theVariable.getDefaultValue();
	// }else
	// defaultValue = this;

	// //document source
	// String docFileURL = (UEngineUtil.isNotEmpty(getPath()) ? getPath() :
	// defaultValue.getTemplateFilePath());

	// if(docFileURL==null)
	// throw new UEngineException(" document file is empty.");

	// ProcessDefinition definition = (instance !=null ?
	// instance.getProcessDefinition() : new ProcessDefinition(){

	// public StringBuffer evaluateContent(ProcessInstance instance, String
	// expression) {
	...
	// docFileURL = (instance != null ? definition.evaluateContent(instance,
	// docFileURL).toString() : docFileURL);

	// InputStream is = new URL(docFileURL).openStream();
	// Workbook workbook = Workbook.getWorkbook(is);

	// String tempFName = defaultValue.getSavingFileName(instance);
	// WritableWorkbook copy = Workbook.createWorkbook(new File(tempFName),
	// workbook);

	// WritableSheet sheets[] = copy.getSheets();
	// //System.out.println(" ExcelSheetActivity:getWIHAddress: start parsing
	// file");
	...
	// WritableCell label = new Label(col,row, definition.evaluateContent(instance,
	// expression).toString(), style);
	// sheet.addCell(label);

	// }else
	// if(expression.indexOf("<%=") > -1 || expression.indexOf("<%=*") > -1){ //if
	// the expression contains any value need to evaluate.
	// //System.out.println("expression:"+expression);
	// l.setString(
	...
	// uploader.connect(defaultValue.getFtpAddress(), defaultValue.getFtpUserId(),
	// defaultValue.getFtpPassword());
	// uploader.uploadFile(tempFName, defaultValue.getFtpDirectory() + "/" +
	// tempFName);
	...
	// public boolean doCommand(ProcessInstance instance, String variableKey) throws
	// Exception {
	// ProcessVariable theVariable =
	// instance.getProcessDefinition().getProcessVariable(variableKey);
	// FileContext defaultValue = (FileContext)theVariable.getDefaultValue();

	// if(defaultValue.getTemplateFilePath()!=null &&
	// defaultValue.getTemplateFilePath().endsWith(".xls")){
	// //parse the completed xsl file and reflect the changes to the corresponding
	// process variables.//
	...
	// //template xls file
	// String urlStr = definition.evaluateContent(instance,
	// defaultValue.getTemplateFilePath()).toString();
	...
	// if(expression.startsWith("<%=*")){ //if the expression contains any value
	// need to evaluate.
	...
	// new UEngineException("File result is not found. You might never open or edit
	// a required document.");
	// }
